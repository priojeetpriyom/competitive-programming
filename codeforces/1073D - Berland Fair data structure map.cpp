...
while(mapp.size() && left>= (*mapp.begin()).first) {
  while(mapp.size() && (*(--mapp.end())).first > left ) {
    ll temp = (*(--mapp.end())).first;
    ara.erase(iters[(*(--mapp.end())).second ]);
    mapp.erase(--mapp.end());
    sum-= temp;
  ...
  // cout<<"mapp "<<mapp.size()<<" left "<<left<<" ans "<<ans<<endl;
}
