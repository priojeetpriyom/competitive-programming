...
SieveOfEratosthenes(1000000);
    ...
    while(true) {
        ...
        for(auto p: divs)
        {
            ...
            if(prev==-1)
                prev == p.second;
            else if(prev!= p.second)
                all_equal=0;
            prev = p.second;
            // cout<<p.second<<" ";
        }
        // cout<<" divs has_odd "<<has_odd <<endl;
        // getchar();
        if(f)
            break;
        ...
        //     mx++;
        // }
        // cout<<"mx "<<mx<<" log"<<log2(mx)<<endl;
        if(has_odd || (!all_equal || (pow(2, (int)log2(mx)) != mx)))
        {
            cnt+=1;
            if(pow(2,(int)log2(mx)) != mx)
            mx = pow(2, floor(log2(mx))+1);
            // for(auto p: divs)
            //     divs[p.first] = mx;
