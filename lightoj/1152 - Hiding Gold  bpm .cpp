...
const int MAXN1 = 202; //nodes in set a
const int MAXN2 = 202; //nodes in set b
const int MAXM = 4004; //number of edges
        ...
        for1(i, n) {
            ...
            for1(j, m) {
                if(grid[i][j]== '*') {
                    ...
                    if(grid[i+1][j]=='*') {
                        if(!cur) {
                            addEdge(cnt_idx[i][j], cnt_idx[i+1] [j]);
                        } else {
                            addEdge(cnt_idx[i+1][j], cnt_idx[i] [j]);

                        }
                    }
                    if(grid[i][j+1]=='*') {
                        if(!cur) {
                            addEdge(cnt_idx[i][j], cnt_idx[i] [j+1]);

                        } else {
                            addEdge(cnt_idx[i][j+1], cnt_idx[i] [j]);
