...
int main() {
    ...
    for1(tc, t) {
        ...
        for0(i, m) {
            siii(male[i].height, male[i].age, male[i].divorced);
        }

        for0(i, f) {
            siii(female[i].height, female[i].age, female[i].divorced);
        }

        for0(i, m) {
            for0(j, f) {
                if((abs(male[i].height - female[j].height)<=12 ) && (abs(male[i].age - female[j].age)<=5 ) && (male[i].divorced == female[j].divorced)) {
                    addEdge(i, j);
                }
            }
        }
